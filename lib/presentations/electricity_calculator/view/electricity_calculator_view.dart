...
import '../../../extensions/size_box.dart';
import '../../../gen/assets.gen.dart';
import '../controller/electricity_calculator_control.dart';

class ElectricityCalculator extends StatelessWidget {
  final controller = Get.put(ElectricityCalculatorController());

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: AppColors.kWhite,
      bottomNavigationBar: Obx(() => controller.isBannerVisible.value
          ? SizedBox(
        width: double.infinity,
        child: controller.bannerAdController.getBannerAdWidget('ad1'),
      )
          : SizedBox.shrink()),
      appBar: PreferredSize(
        preferredSize: Size(0, 70),
          ...
          child: Column(
            ...
            children: [
              ...
              ListView.builder(
                shrinkWrap: true,
                physics: const NeverScrollableScrollPhysics(),
                itemCount: controller.namesTitle.length,
                itemBuilder: (context, index) {
                  return CustomContainer(
                    ...
                    child: ListTile(
                      dense: true,
                      leading: Image.asset(controller.images[index], scale: 18),
                      title: regularTextWidget(
                        textTitle: controller.namesTitle[index],
                        textSize: 18,
                        textColor: AppColors.kWhite,
                        fontWeight: FontWeight.w500,
                      ),
                      subtitle: regularTextWidget(
                        textTitle: controller.subTitle[index],
                        textSize: 14,
                        textColor: AppColors.kWhite,
                      ),
                      trailing: Icon(
              ...
              CustomContainer(
                ...
                child: ListTile(
                  dense: true,
                  onTap: () => controller.openMetering(),
                  leading: Image.asset(Assets.metering.path, scale: 18),
                  title: regularTextWidget(
                    textTitle: controller.meteringName[0]['name'],
                    textSize: 18,
                    textColor: AppColors.kWhite,
                    fontWeight: FontWeight.w500,
                  ),
                  subtitle: regularTextWidget(
                    textTitle: controller.meteringName[0]['subtitle'],
                    textSize: 14,
                    textColor: AppColors.kWhite,
                  ),
                  trailing: Icon(
